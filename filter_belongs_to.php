...
if (!empty($_POST['belongs_to'])) {
    if ($_POST['belongs_to'] == 'Department') {
        if (!empty($_POST['schoolName'])) {
            $conditions[] = "hd.school_id = ?";
            $params[] = $_POST['schoolName'];
        }
        if (!empty($_POST['departmentName'])) {
            $conditions[] = "hd.department_id = ?";
            $params[] = $_POST['departmentName'];
        }
    } elseif ($_POST['belongs_to'] == 'School') {
        if (!empty($_POST['onlySchoolName'])) {
            $conditions[] = "hd.school_id = ?";
            $params[] = $_POST['onlySchoolName'];
        }
    } elseif ($_POST['belongs_to'] == 'Administration') {
        if (!empty($_POST['section'])) {
            $conditions[] = "hd.section_id = ?";
